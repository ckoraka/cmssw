...
namespace egamma {
  ...
  private:
    Vec3f momentum;    // 3D momentum vector
    Vec3f position;    // 3D position vector
    const int charge;  // Particle charge
  public:
    // Constructor
    constexpr FreeTrajectoryState(const Vec3f& p, const Vec3f& pos, const int q)
        : momentum(p), position(pos), charge(q) {}

    constexpr Vec3f get_momentum() const { return momentum; }
    constexpr Vec3f get_position() const { return position; }
  ...
  ftsFromVertexToPoint(TAcc const& acc,
                       const Vec3f& xmeas,    // Measured point
                       const Vec3f& xvert,    // Vertex point
    ...
    using T = Vec3f::value_type;
    //
    // Calculate the difference between measurement and vertex positions
    const Vec3f xdiff = cms::alpakatools::math::xmy(xmeas, xvert);  //= xmeas - xvert;
    ...
    const Vec3f mom = cms::alpakatools::math::ax(scale, xdiff);
    ...
    const T curv = (BInTesla * 0.29979f * 0.01f) / pt;

    // Calculate the sine and cosine of the rotation angle
    const T sa = 0.5f * xdiff.partial_norm(acc) * curv * static_cast<T>(charge);
    const T ca = alpaka::math::sqrt(acc, 1.f - sa * sa);

    // Rotate momentum vector in the xy-plane
    const T pxNew = ca * pxOld + sa * pyOld;
    const T pyNew = -sa * pxOld + ca * pyOld;
    //
    const Vec3f pNew(pxNew, pyNew, pz);
