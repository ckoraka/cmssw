	...
	namespace Propagators {
										...
										const double cachedCDPhi)
		{
			if (select) {
				...
				return Vec3d(
					point[0] + (-sinPhi0 * (1.0 - cachedCDPhi) + cosPhi0 * cachedSDPhi) * o,
				...
				return Vec3d(
					point[0] + (cosPhi0 - st * 0.5 * rho * sinPhi0) * st,
										 ...
										 const double cachedCDPhi)
		{
			if (select) {
				return Vec3d(
					cosPhi0 * cachedCDPhi - sinPhi0 * cachedSDPhi,
				...
				return Vec3d(
					cosPhi0 - (sinPhi0 + 0.5 * cosPhi0 * dph) * dph,
			...
			bool& solExists)
		{
			...
			const bool validSolution =
				!min_cosTheta_flag &&
				!(((propDir == PropagationDirection::alongMomentum) &&
				   (pathLength < 0.)) ||
				  ((propDir == PropagationDirection::oppositeToMomentum) &&
				   (pathLength > 0.)) ||
			...
			position = positionInHelix(
				cachedDPhi_flag, pathLength, point,
				curvature, cosPhi0, sinPhi0,
				cosTheta, sinTheta,
				cachedS, cachedSDPhi, cachedCDPhi
			);

			dir = directionInHelix(
				cachedDPhi_flag, pathLength, curvature,
				cosPhi0, sinPhi0, cosTheta, sinTheta,
				cachedSDPhi, cachedCDPhi
			);
